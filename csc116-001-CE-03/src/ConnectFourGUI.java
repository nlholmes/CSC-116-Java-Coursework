 ...
 */
 public class ConnectFourGUI extends JFrame implements ActionListener {
    ...
    private int redWins = 0; // total red wins
    private int blackWins = 0; // total black wins
    ...
    // Status panel labels, streaks are really just total wins
    ...
    // Panels
    private JPanel borderPanel;// = new JPanel(new BorderLayout()); // container panel
    private JPanel boardPanel;// = new JPanel(new GridLayout(rows, cols, 10, 10)); // game board panel, with padding
    private JPanel statusPanel;// = new JPanel(new GridLayout(2,2,5,5)); // 2x2 status panel, with padding
    ...
    */
    ...
    public ConnectFourGUI() {
        ...
        borderPanel = new JPanel(new BorderLayout()); // container panel
        boardPanel = new JPanel(new GridLayout(rows, cols, 10, 10)); // game board panel, with padding
        statusPanel = new JPanel(new GridLayout(2,2,5,5)); // 2x2 status panel, with padding
        ...
        redStreakField.setText("0"); // starting value for red wins
        blackStreakField.setText("0"); // starting val
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        // Wins for each player
        //redStreakField.setText("" + redWins); // update red wins field
        //blackStreakField.setText("" + blackWins); // update black wins field
    ...
    public boolean newGameDialog(char winner, boolean tie) {
        if(winner != 'n' || tie) { // if there was a winner or a tie, need to reprompt
            ...
            if (choice == JOptionPane.YES_OPTION) {
                ...
                // Need to set first player to go as red ('r') again and get new streak
                this.turn = 'r'; // sets turn to 'r' again
                ...
                //new ConnectFourGUI();// this makes new panel appear, we just want to change ours
                ...
                // Currently this code does not reset the panels like needed
                for(int i = 0; i < buttons.length; i++) { // rows
                    for(int j = 0; j < buttons[i].length; j++) { // cols
                        buttons[i][j].setBackground(Color.WHITE);
                    }
